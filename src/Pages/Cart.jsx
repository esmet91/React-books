...
import { removeFromCart, increment, decrement } from '../Redux/Slices/itemSlice';

function Cart() {
  const cartItems = useSelector((state) => state.item.itemCart);
  const dispatch = useDispatch();

  const totalCount = useSelector((state) => state.item.totalCount);
        ...
        <>
              ...
              <div key={obj.id} className="cart__item item-cart">
                ...
                <div className="item-cart__counter counter-cart">
                  <div className="counter-cart__container">
                    <button
                      onClick={() => countMinus(obj)}
                      className="counter-cart__button btn -minus">
                      -
                    </button>
                    <span>{obj.count}</span>
                    <button
                      onClick={() => countPlus(obj)}
                      className="counter-cart__button btn -plus">
                      +
                    </button>
                  </div>
                </div>
                <div className="item-cart__price">
                  <p className="item-cart__price-for-one">{(obj.price * obj.count).toFixed(2)} ₽</p>
                ...
                <div className="item-cart__buttons buttons-cart">
                  <div className="buttons-cart__container">
                    ...
                    <button onClick={() => removeItemCart(obj)} className="buttons-cart__remove">
          ...
          <div className="total">
            <p className="total__tota-count">
              <span>Товаров -</span> {totalCount}
            </p>
            <p className="total__tota-price">
              <span>Сумма к оплате -</span> {totalSum} ₽
            </p>
